...
public class Crosshair : MonoBehaviour
{
    ...
    {
        //For objects whos parents contain the script chosen
        if (hit.collider.transform.parent != null && hit.distance <= distance && Input.GetMouseButtonDown(0) && CheckIfAllTrue())
        {
            ...
            if (parent.GetComponent<Farm>() && playerInteractions.assign)
            {
                ...
                playerInteractions.residentText.text = "Okay pefect I will collect food from here now";
            ...
            if (parent.GetComponent<Job>() && playerInteractions.residentFollowing)
            {
                ...
                {
                    playerInteractions.residentText.text = "his job is full, I can't work here";
                }
            ...
            if (parent.GetComponent<IsABreakable>() != null || hit.collider.GetComponent<IsABreakable>() != null)
            {
                if (hit.collider.transform.parent.GetComponent<Health>() != null || hit.collider.transform.GetComponent<Health>() != null || hit.collider.transform.GetComponent<ResidentHealth>() != null)
                {
                    //axe.GetComponent<Tool>().hitter = hit.collider.transform.gameObject;
                    //axe.GetComponent<Tool>().point = hit.point;

                   /// pickaxe.GetComponent<Tool>().hitter = hit.collider.transform.gameObject;
                    //pickaxe.GetComponent<Tool>().point = hit.point;
                }
            }
        }

        //For objects whoself contain the script chosen
        if (hit.collider.transform != null && hit.distance <= distance && Input.GetMouseButtonDown(0) && CheckIfAllTrue())
        {
        ...
        //For objects who need the "E" button clicked
        if ((hit.collider.transform.parent != null || hit.collider.transform != null) && hit.distance <= distance && Input.GetKeyDown(KeyCode.F) && CheckIfAllTrue())
        {
            Transform GameObject = hit.collider.transform;
            Transform parent = hit.collider.transform.parent;
            PickUp pickUp = null;
            ...
            /*if (parent.GetComponent<PickUp>() != null || GameObject.GetComponent<PickUp>() != null)
            {
                pickUp = hit.transform.GetComponent<PickUp>();
                PickUp pickUpParent;
                if (hit.transform.parent != null)
                {
                    pickUpParent = hit.transform.parent.GetComponent<PickUp>();
                    if (pickUpParent != null)
                    {
                        pickUp = hit.transform.parent.transform.GetComponent<PickUp>();
                    }
                }
            }*/
    ...
    private void PopulateCanvasObjects()
    {
