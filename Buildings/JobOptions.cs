...
public class JobOptions : MonoBehaviour
{
    ...
    public void Fire()
    {
        ...
        //job.StatMultiplier -= resident.GetComponent<ResidentStats>().Stats[3];
        //job.SendStats();
    ...
    {
        GameObject job = resident.GetComponent<ResidentScheudle>().job;

        try
        {
            if (job.GetComponent<Farm>() != null) Destroy(resident.GetComponent<Farmer>());
            else if (job.GetComponent<Mine>() != null) Destroy(resident.GetComponent<Miner>());
            else if (job.GetComponent<Lumbermill>() != null) Destroy(resident.GetComponent<LumberWorker>());
            else if (job.GetComponent<Messhall>() != null) Destroy(resident.GetComponent<Chef>());
        }
        catch { };
    }
}
